...
const PROMPTS = {
    ...
    Dog: {
        ...
        video: `你是一位狗狗行為專家。請分析接下來的狗狗影片畫面。 1. 以繁体中文描述狗狗的關鍵肢體語言（例如：搖尾巴、耳朵前傾、露牙、遊戲鞠躬）。 2. *僅*根據視覺線索判斷狗狗最可能的狀態。請從以下選項中選擇：Happy, Playful, Anxious, Warning。如果狀態是冷靜或不明確，請選擇 "Unknown"。 3. 以繁體中文為飼主提供有用的建議。請以單一 JSON 物件格式回應。`,
...
// FIX: Updated function signature to accept an `apiKey` parameter to match the function call in App.tsx.
export async function translateAudio(audioBlob: Blob, subject: Subject, apiKey: string): Promise<TranslationResult> {
  // FIX: Validate the passed `apiKey` instead of relying on environment variables.
  if (!apiKey) {
    throw new Error("API Key is not provided");
  }

  // FIX: Use the provided `apiKey` to initialize the GoogleGenAI client.
  ...
  try {
    ...
    // FIX: Throw the error so it can be caught and handled in the component.
    throw error;
  }
}

// FIX: Updated function signature to accept an `apiKey` parameter to match the function call in App.tsx.
export async function analyzeVideoFrames(imageBase64s: string[], subject: Subject, apiKey: string): Promise<TranslationResult> {
  // FIX: Validate the passed `apiKey` instead of relying on environment variables.
  if (!apiKey) {
    throw new Error("API Key is not provided");
  }
